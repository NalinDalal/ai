"""
lstm.py - LSTM Cell from Scratch (NumPy)
Based on Hochreiter & Schmidhuber (1997)
"""
...
class LSTMCell:
    ...
    def backward(self, dh_next, dC_next, dy):
        ...
        self.dW_xi = (
            np.dot(di_raw, self.x.T) / batch if batch > 1 else di_raw * self.x.T
        )
        self.dW_hi = (
            np.dot(di_raw, self.h_prev.T) / batch
            if batch > 1
            else di_raw * self.h_prev.T
        )
        self.db_i = (
            np.sum(di_raw, axis=1, keepdims=True) / batch if batch > 1 else di_raw
        )

        self.dW_xf = (
            np.dot(df_raw, self.x.T) / batch if batch > 1 else df_raw * self.x.T
        )
        self.dW_hf = (
            np.dot(df_raw, self.h_prev.T) / batch
            if batch > 1
            else df_raw * self.h_prev.T
        )
        self.db_f = (
            np.sum(df_raw, axis=1, keepdims=True) / batch if batch > 1 else df_raw
        )

        self.dW_xo = (
            np.dot(do_raw, self.x.T) / batch if batch > 1 else do_raw * self.x.T
        )
        self.dW_ho = (
            np.dot(do_raw, self.h_prev.T) / batch
            if batch > 1
            else do_raw * self.h_prev.T
        )
        self.db_o = (
            np.sum(do_raw, axis=1, keepdims=True) / batch if batch > 1 else do_raw
        )

        self.dW_xc = (
            np.dot(dC_tilde_raw, self.x.T) / batch
            if batch > 1
            else dC_tilde_raw * self.x.T
        )
        self.dW_hc = (
            np.dot(dC_tilde_raw, self.h_prev.T) / batch
            if batch > 1
            else dC_tilde_raw * self.h_prev.T
        )
        self.db_c = (
            np.sum(dC_tilde_raw, axis=1, keepdims=True) / batch
            if batch > 1
            else dC_tilde_raw
        )
